 ...
 * @author leo
 */
@Tag(name = "管理后台 - 低代码-部署接口")
@RestController
@RequestMapping("/lowcode/deploy-api")
@Validated
public class AdminDeployApiController {
    ...
    private void assertAccess(QuerierEditorDataVO data, String method) {
        if (!CollectionUtil.contains(data.getChannels(), DeployApiChannel.ADMIN.getValue()) ||
    ...
    @PostMapping(value = "/{apiName}/export")
    @Operation(summary = "调用部署接口-查询export")
    public void deployApiExport(
            @PathVariable String apiName, @RequestParam(required = false) String apiCode,
            @RequestBody QueryDomainParams params, HttpServletResponse response) {
        var apiData = this.queryDomainDeployApi.getApiData(apiName, apiCode);
        assertAccess(apiData, "export");
        QueryDomainExecutor executor = this.queryDomainExecutorFactory.createExecutor(apiData.getDataSourceId(), apiData.getQueryXml());
        executor.export(params, response);
    }
